...
現行BannerAdWidgetはAdaptive Bannerへ移行し、即採用する。端末幅に追従させる一方、広告が必要以上に高くなって地図や主要コンテンツを圧迫しないことを必須条件とする。利用可能幅からanchored adaptive banner sizeを取得し、取得できない場合は安全に非表示または従来サイズへフォールバックする。広告未ロード時は不要な空白を残さない。
