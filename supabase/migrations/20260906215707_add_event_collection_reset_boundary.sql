...
-- イベントリセット境界
--
-- place_visits は訪問事実として保持する。
-- リセット以前の訪問は、その後再送されても再獲得には使用しない。
-- ============================================================

create table if not exists public.event_collection_resets (
...
alter table public.event_collection_resets
  enable row level security;

-- クライアントからの直接操作は禁止。
-- SECURITY DEFINER RPCだけが更新する。
revoke all
on table public.event_collection_resets
...
-- ============================================================
-- 獲得RPC
-- 20260911版を維持し、リセット境界判定だけを追加する。
-- ============================================================
-- ============================================================
-- collection RPC: seichi lookup を place_id ベースへ変更
-- ============================================================

-- 既存の緯度経度完全一致ではなく、
-- seichi.place_id と event_contents.place_id を正規の結合キーとして使う。
