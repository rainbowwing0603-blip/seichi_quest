-- ============================================================
-- Legacy seichi dependency removal
--
-- New collection identity is event_contents.id.
-- This migration refuses to remove the legacy column/table unless every
-- existing collection row is already backed by the generic structure.
-- ============================================================
...
-- SECURITY DEFINER + search_path = '' のため、
-- PostGIS関数・geography型は実DB配置に合わせて public スキーマで完全修飾する。
-- ============================================================
...
begin
  ...
  --
  -- 判定基準:
  --   新規訪問 = 保存された place_visits.visited_at
  --   再送     = 最初に保存された place_visits.visited_at
  -- ----------------------------------------------------------
  return query
  with eligible as (
    select
...
alter table public.collection_history
  drop constraint if exists collection_history_user_event_seichi_unique,
  drop constraint if exists collection_history_seichi_id_fkey;

drop index if exists public.collection_history_user_event_seichi_unique;
